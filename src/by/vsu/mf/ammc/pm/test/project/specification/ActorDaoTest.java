...
public class ActorDaoTest {
	...
	public static void main(String args[]) throws PersistentException, SQLException
	{
		...
		ConnectionPool pool = ConnectionPool.getInstance();
 		...
 		//read 
 		Actor readActor = dao.read(42005); 		
 		
        assert readActor.getName().equals(createActor.getName()) &&
			readActor.getProject().getId().equals( createActor.getProject().getId())
               : "Failed on read actor";
        		
        //update
        createActor.setName("feoi");
        dao.update(createActor);
        readActor = dao.read(42005);
        assert readActor.getName().equals(createActor.getName()) &&
			readActor.getProject().getId().equals( createActor.getProject().getId())
               : "Failed on update actor";        		
        		
        //delete
        dao.delete(42005);
        assert dao.read(42005) == null : "Failed on delete actor";
